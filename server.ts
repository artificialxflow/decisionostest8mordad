...
async function startServer() {
  ...
  app.use('/api/v1', (req, res, next) => {
    ...
    res.setHeader('X-RateLimit-Remaining', String(Math.max(0, 120 - entry.count)));
    if (entry.count > 120) {
  ...
  // Request validation helper for JSON bodies
  function requireFields(fields: string[]) {
    return (req: express.Request, res: express.Response, next: express.NextFunction) => {
      for (const f of fields) {
        if (req.body?.[f] === undefined || req.body?.[f] === null || req.body?.[f] === '') {
          return res.status(400).json({ error: `فیلد الزامی: ${f}` });
        }
      }
      next();
    };
  ...
  // JWT placeholder: Authorization Bearer header accepted for future auth
  app.use('/api/v1', (req, _res, next) => {
    const auth = req.headers.authorization;
    if (auth?.startsWith('Bearer ')) {
      (req as express.Request & { token?: string }).token = auth.slice(7);
    }
    next();
  });

  // CSRF note: SPA same-origin; cookie-based CSRF to be added with real session auth
  ...
  // Health check (versioned + legacy)
  app.get('/api/v1/health', (req, res) => {
    res.json({ status: 'ok', version: 'v1', hasGeminiKey: !!process.env.GEMINI_API_KEY });
  });
  ...
  app.get('/api/v1/workspaces/:id', (req, res) => {
    ...
    if (!ws) return res.status(404).json({ error: 'Workspace یافت نشد.' });
    res.json(ws);
  });
  ...
  // Subscriptions & Invoices (skeleton)
  ...
  app.post('/api/v1/cases', requireFields(['title']), (req, res) => {
    const body = req.body;
    const newCase: CaseItem = {
      ...
      assignedExpertId: body.assignedExpertId || 'usr-1',
      deadline: body.deadline,
      workspaceId: body.workspaceId || workspaces[0]?.id,
  ...
  // --- NOTES ---
  app.get('/api/v1/cases/:id/notes', (req, res) => {
    const list = notes.filter(n => n.caseId === req.params.id);
    res.json(list);
  });

  app.post('/api/v1/cases/:id/notes', (req, res) => {
...
`;
  ...
  app.post('/api/v1/cases/:id/ai-analyze', async (req, res) => {
    try {
